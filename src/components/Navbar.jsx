...
function Navbar() {
	...
	return (
		<nav className="bg-white shadow-lg sticky top-0 z-50 py-4 px-4 md:px-8 border-b border-gray-200">
			<div className="flex items-center justify-between">
				<div className="flex items-center">
					<img src={logoUrl} alt="IATPL Logo" className="h-12 w-auto mr-4 md:h-16 md:mr-8" />
				</div>
				...
				<ul className="hidden md:flex space-x-10">
					{navLinks.map(link => (
						<li key={link.to}>
							<Link
								to={link.to}
								className={`uppercase font-bold tracking-wide text-lg transition-colors px-2 ${location.pathname === link.to ? 'text-red-600' : 'text-black hover:text-blue-600'}`}
							>
								{link.label}
							</Link>
						</li>
			...
			{/* Mobile menu */}
			{menuOpen && (
				<div className="md:hidden mt-2 bg-white rounded-lg shadow-lg p-4 absolute left-0 right-0 top-20 z-40">
					<ul className="flex flex-col space-y-4">
						{navLinks.map(link => (
							<li key={link.to}>
								<Link
									to={link.to}
									className={`uppercase font-bold tracking-wide text-lg transition-colors px-2 block ${location.pathname === link.to ? 'text-red-600' : 'text-black hover:text-blue-600'}`}
